...
class Mastodon_api {
	...
	private function _post ($url,$data = array()) {
		...
		if (count($data)) {
			...
			$postfields = str_replace("%5B0%5D", "%5B%5D", $postfields);
			$postfields = str_replace("%5B1%5D", "%5B%5D", $postfields);
			$postfields = str_replace("%5B2%5D", "%5B%5D", $postfields);
			$postfields = str_replace("%5B3%5D", "%5B%5D", $postfields);
			
			$parameters[CURLOPT_POSTFIELDS] = $postfields;
		}
